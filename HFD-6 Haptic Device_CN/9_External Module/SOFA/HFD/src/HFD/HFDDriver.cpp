...
void* __stdcall stateCallback(void* userData)
{
    ...
    while (!driver->m_bThreadFinish)
    {
        ...
        {
            clafGetPosition(&(driver->m_Pos[0]), &(driver->m_Pos[1]), &(driver->m_Pos[2]));

            clafGetOrientationFrame(driver->m_Orientation);

            clafGetDeltaJointAngles(&(driver->m_BaseAngle[0]), &(driver->m_BaseAngle[1]),
                                    &(driver->m_BaseAngle[2]));

            clafGetWristJointAngles(&(driver->m_ToolAngle[0]), &(driver->m_ToolAngle[1]),
            ...
            driver->m_fButtonPress = clafGetButton(0);
            ...
            {
                clafSetForce(driver->omni_force[0], driver->omni_force[1], driver->omni_force[2]);
...
void HFDDriver::initDevice()
{
    ...
    //! open the CLAF haptic device.
    error = clafOpen();
    if (error < 0)
    {
        msg_warning() << "CLAF Device Open Failed!";
    ...
    //! initialize the CLAF haptic device.
    error = clafInit();
    if (error == 0)
    {
        msg_warning() << "CLAF Device Init Failed!";
        sofa::core::objectmodel::BaseObject::d_componentState.setValue(
            sofa::core::objectmodel::ComponentState::Invalid);
        return;
    }
    m_bInit = true;
        ...
        error = clafCalibrateDevice();
    if (error == 0)
    {
        msg_warning() << "Calibrate the CLAF Haptic Device Failed!";
        return ;
    }
    m_bCalibrate = true;

    error = clafEnableForce(CLAF_ON);
    if (error == 0)
    {
    ...
    clafEnableExpertMode();

    error = clafEnableDevice(true);
    if (error == 0)
    {
    ...
    error = clafSetGravityCompensation(CLAF_ON);
    if (error == 0)
    {
    ...
    error = clafStartThread(stateCallback, this, 1);
    if (error == 0)
    {
        msg_warning() << "Refresh thread of CLAF device faile to create!";
        sofa::core::objectmodel::BaseObject::d_componentState.setValue(
            sofa::core::objectmodel::ComponentState::Invalid);
        return;
    }
