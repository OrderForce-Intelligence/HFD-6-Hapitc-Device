...
HFDEmulatorTask::HFDEmulatorTask(HFDEmulator* ptr, CpuTask::Status* pStatus)
    :CpuTask(pStatus)
    , m_driver(ptr)
{
...
HFDEmulatorTask::MemoryAlloc HFDEmulatorTask::run()
{
    ...
    {
        TaskScheduler::getInstance()->addTask(new HFDEmulatorTask(m_driver, &m_driver->_simStepStatus));
...
void HFDEmulator::computeTransform()
{

}


void HFDEmulator::applyTranslation(sofa::type::Vec3 translation)
{
    lockPosition.lock();
...
void HFDEmulator::worldToLocal(sofa::type::Vec3& vector)
{
    vector = d_orientationTool.getValue().rotate(vector);
}

void HFDEmulator::moveJoint1(SReal value)
{
    m_hapticData.angle1[0] += value;
    computeTransform();
}

void HFDEmulator::moveJoint2(SReal value)
{
    m_hapticData.angle1[1] += value;
    computeTransform();
}

void HFDEmulator::moveJoint3(SReal value)
{
    m_hapticData.angle1[2] += value;
    computeTransform();
}

void HFDEmulator::moveGimbalAngle1(SReal value)
{
    m_hapticData.angle2[0] += value;
    computeTransform();
}

void HFDEmulator::moveGimbalAngle2(SReal value)
{
    m_hapticData.angle2[1] += value;
    computeTransform();
}

void HFDEmulator::moveGimbalAngle3(SReal value)
{
...
void HFDEmulator::handleEvent(core::objectmodel::Event *event)
{
    ...
    else if (sofa::core::objectmodel::KeypressedEvent::checkEventType(event))
    {
        ...
        //msg_info() << "HFDEmulator handleEvent gets character '" << ke->getKey() << "'. ";
...
int HFDEmulatorClass = core::RegisterObject("Driver allowing interfacing with HFD haptic devices.")
.add< HFDEmulator >()
;
