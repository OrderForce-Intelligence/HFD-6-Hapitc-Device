...
******************************************************************************/
#ifndef SOFA_COMPONENT_CONSTRAINTSET_FORCEPOINTACTUATOR_INL
#define SOFA_COMPONENT_CONSTRAINTSET_FORCEPOINTACTUATOR_INL

#include "ForcePointActuator.h"
...
void ForcePointActuator<DataTypes>::draw(const VisualParams* vparams)
{
    ...
    static const type::RGBAColor color(0,0,0.8,1);
    for(unsigned int i=0; i<d_indices.getValue().size(); i++)
        if(d_indices.getValue()[i]<m_state->getSize())
        {
