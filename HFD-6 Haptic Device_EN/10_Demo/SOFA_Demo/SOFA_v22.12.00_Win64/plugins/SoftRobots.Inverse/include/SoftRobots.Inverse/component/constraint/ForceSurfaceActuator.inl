...
******************************************************************************/
#ifndef SOFA_COMPONENT_CONSTRAINTSET_FORCESURFACEACTUATOR_INL
#define SOFA_COMPONENT_CONSTRAINTSET_FORCESURFACEACTUATOR_INL

#include "ForceSurfaceActuator.h"
...

{
    ...
    WriteAccessor<Data<vector<Real>>> force = d_force;
    force.resize(1);
...
void ForceSurfaceActuator<DataTypes>::initData()
{
    ...
    /// topology
    WriteAccessor<Data<VecCoord> > positions = d_positions;
...
{
    ReadAccessor<Data<vector<Coord> > >     positions = d_positions;
    ReadAccessor<Data<VecCoord> >           centers   = d_centers;
    ReadAccessor<Data<vector<Real>>>        radii     = d_radii;
...
{
    WriteAccessor<Data<vector<Real>>> force = d_force;
    WriteAccessor<Data<vector<Real>>> displacement = d_displacement;
...
{
    WriteAccessor<Data<vector<Coord>>> centers = d_centers;
    ReadAccessor<Data<vector<Real>>> displacement = d_displacement;
    ReadAccessor<Data<vector<Deriv>>> directions = d_directions;
...
void ForceSurfaceActuator<DataTypes>::drawForces(const VisualParams* vparams)
{
    ...
    ReadAccessor<Data<vector<Real>>> force = d_force;
...
void ForceSurfaceActuator<DataTypes>::drawTriangles(const VisualParams* vparams)
{
    ...
    vparams->drawTool()->drawTriangles(points, type::RGBAColor(0.6f,0.0f,0.0f,1.0f));
}
...
void ForceSurfaceActuator<DataTypes>::drawLines(const VisualParams* vparams)
{
    ...
    vparams->drawTool()->drawLines(points, 1.0f, type::RGBAColor(0.2f,0.0f,0.0f,1.0f));
}
