...
def createScene(rootNode):
    ...
    rootNode.addObject('RequiredPlugin', name='SofaPlugins', pluginName=['SofaSparseSolver','SofaDeformable','SofaImplicitOdeSolver','SofaRigid','SofaConstraint','SofaBoundaryCondition'])
    ...
    goal.addObject('MechanicalObject', name='dofs', template='Rigid3', showObject=True, drawMode=2, showObjectScale=1, position=[30.0, 0.0, 0.0,0,0,0,1] )
    ...
    model.addObject('MeshTopology', position=[[0, 0, 0], [5, 0, 0], [10, 0, 0], [15, 0, 0], [20, 0, 0]], lines=[[0, 1], [1, 2], [2, 3], [3, 4]])

    teta=3.14/8;
    model.addObject('MechanicalObject', template='Rigid3', name='dofs',
                        position=[[0, 0, 0, 0, 0, 0, 1], [5, 0, 0, 0, 0, 0, 1], [10, 0, 0, 0, 0 ,0 ,1], [15, 0, 0 ,0 ,0, 0, 1], [20, 0, 0, 0, 0, 0, 1]],
                        rest_position=[[0, 0, 0, 0, 0, 0, 1], [5, 0, 0, 0, 0, 0, 1], [10, 0, 0, 0, 0 ,0 ,1], [15, 0, 0 ,0 ,0, 0, 1], [19, 2, 0, 0, 0, sin(teta), cos(teta)]],
    ...
    # Ex: if SlidingActuator simulates a single dof (e.g. translation along x), the other dofs must be fixed using this component
    ...
    effector.addObject('PositionEffector', indices=0, effectorGoal=goal.dofs.findData('position').getLinkPath(), useDirections=[1, 1, 0]) # No need to follow the target in z direction because the beam working space is in the [x,y] plane
